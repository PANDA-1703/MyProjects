...
class Stratostat:
    def __init__ (self, parent, width=400, height=400, title="Стратостат", resizable=(True,True)):
    ...
    def draw_widgets(self): # Отрисовка виджетов
        ...
        self.ro_air_entry.grid(row=3, column=1, padx=5)
        self.ro_gas_entry.grid(row=4, column=1, padx=5)
    ...
    def bt_ok(self): # Функция основных вычислений
        ...
        m = double(self.m_entry.get())  # Давление атм. воздуха на уровне моря
        ro_air = double(self.ro_air_entry.get())  # Температура атмосферного воздуха в К
        ro_gas = double(self.ro_gas_entry.get())  # Температура газа в К
        ...
        V=(4/3)*pi*R**3 # Объём 
        ro_sum=ro_gas+m/((4/3)*pi*R**3) # Суммарная плотность материала стратостата, массы гелия и нагрузки
        ...
        plt.title("\n Характеристики  подъёма стратостата  \n Объём: %s м^3. Масса : %s кг. \n Подъёмная сила: %s kН. " % (
            round(V, 0), m, round(0.001*g*ro_air*V, 0)))
